    ...
    public class AddUserModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                List<Institution> institutionSet = await institutionDbSet?.ToListAsync();
                if (institutionSet != null)
                {
                    ApplicationUser currentUser = await _userManager.GetUserAsync(HttpContext.User);
                    if (currentUser != null)
                    {
                        foreach(Institution i in institutionSet)
                        {
                        ...
                        if (result.Succeeded)
                        {
                            await _userManager.AddToRoleAsync(user, "College");
